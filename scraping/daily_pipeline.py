...
class DailyPipeline:
    ...
    def __init__(self, api_key, model=None, template_path=None, data_dir="data", days_lookback=1):
        """
        ...
        self.model = model
        self.template_path = template_path or os.path.join(os.path.dirname(__file__), "template.txt")
    ...
    def load_existing_data(self):
        ...
        try:
            df = pd.read_csv(os.path.join(self.data_dir, "raw", "afectaciones_electricas_cubadebate_filter_2025.csv"), encoding='utf-8-sig')
            logger.info(f"Datos existentes cargados: {len(df)} registros")
            return df
        except Exception as e:
            logger.warning(f"Error al cargar datos existentes: {e}. Se utilizará un DataFrame vacío.")
            return pd.DataFrame()
    ...
    def get_latest_articles(self, max_pages=5):
        ...
        """
        logger.info(f"Iniciando scraping de artículos recientes (últimos {self.days_lookback} días)")
        
        existing_df = self.existing_data
        existing_urls = set(existing_df['Enlace'].tolist()) if not existing_df.empty and 'Enlace' in existing_df.columns else set()
        
        min_date = (self.today - timedelta(days=self.days_lookback)).strftime("%Y-%m-%d")
        ...
        for page_num in range(1, max_pages + 1):
            ...
            try:
                ...
                for article in articles:
                    ...
                    excerpt = article.find('div', class_='excerpt').get_text(strip=True) if article.find('div', class_='excerpt') else ""
                
                    if any(keyword.lower() in title.lower() or keyword.lower() in excerpt.lower() for keyword in ["eléctrica", "Unión Eléctrica", "apagón", "UNE", "corte de luz", "generación eléctrica", "MW", "Felton", "termoeléctrica"]):
                        logger.info(f"Artículo encontrado: {title}")
                        
                        article_content = scrape_article_content(link, {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'})
                        if article_content:
                            
                            article_date = article_content.get('Fecha', '').split('T')[0]
                            if article_date >= min_date:
                                found_recent = True
                                articles_data.append(article_content)
                                logger.info(f"Artículo agregado: {title} - {article_date}")
                            else:
                                logger.debug(f"Artículo demasiado antiguo: {title} - {article_date}")
                        
                        time.sleep(1)  
            
  
                if not found_recent and page_num > 1:
                    logger.info(f"No se encontraron más artículos recientes en la página {page_num}. Finalizando.")
                    break
        ...
        if not new_articles_df.empty:
            ...
            logger.info(f"Se encontraron {len(new_articles_df)} artículos nuevos. Guardados en {daily_file}")
            
            updated_df = pd.concat([new_articles_df,existing_df], ignore_index=True)
            updated_df.to_csv(os.path.join(self.data_dir, "raw", "afectaciones_electricas_cubadebate_filter_2025.csv"), index=False, encoding='utf-8-sig')
            logger.info(f"raw/afectaciones_electricas_cubadebate_filter_2025.csv actualizado")
        else:
    ...
    def update_main_json(self):
        ...
        """
        main_json_path = os.path.join(self.data_dir, "datos_electricos_organizados.json")
        daily_dir = os.path.join(self.data_dir, "daily", self.today.strftime("%Y-%m-%d"))
        ...
        if not os.path.exists(json_processed):
            logger.warning(f"No se encontró el archivo JSON procesado en {json_processed}")
        ...
        try:
            ...
            for year, months in new_data.items():
                ...
                for month, items in months.items():
                    ...
                    if items['enlace'] not in existing_urls:
                        main_data[year][month].extend(items)
                        items_added += len(items)
 
            
            logger.info(f"Se agregaron {items_added} elementos nuevos al JSON principal")
            ...
            try:
                ...
                processed_path = os.path.join(self.data_dir, "processed", "datos_electricos_organizados.json")
    ...
    def _process_and_save_day(self, df, date_str=None):
        ...
        try:
            os.makedirs(os.path.join(self.data_dir,'daily',self.today.strftime("%Y-%m-%d")),exist_ok=True)
            daily_output_dir = os.path.join(self.data_dir, "daily",self.today.strftime("%Y-%m-%d"))
            os.makedirs(daily_output_dir, exist_ok=True)
            
        
            temp_csv_path = os.path.join(daily_output_dir, f"temp_articulos_{str(self.today)}.csv")
            ...
            if result == 0:
                logger.info(f"Extracción JSON completada con éxito para {date_str}")
                return True
            else:
                logger.error(f"Error durante la extracción JSON para {date_str}")
                return False
                
        except Exception as e:
            logger.error(f"Error en el procesamiento de artículos de {date_str}: {e}")
            return False
    ...
    def run(self,analize_all=False):
        ...
        if not analize_all:
            ...
            if articles.empty:
                logger.warning(f"No se encontraron nuevos artículos en los últimos {self.days_lookback} días")
            ...
            logger.info(f"Se encontraron {len(articles)} artículos nuevos para procesar")
            
            self.process_new_articles(articles)
            
            logger.info("Pipeline completado exitosamente")
            return True
        logger.info("iniciando la Creación de los JSON para todos los articulos filtrados")
        path =os.path.join(self.data_dir, "raw", "afectaciones_electricas_cubadebate_filter_2025.csv")
        extractor = CreateJson(
                path_df=path,
                path_template=self.template_path,
                url_llm="https://api.fireworks.ai/inference/v1/chat/completions",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--days_lookback', type=int, default=1, help='Number of days to look back')
    parser.add_argument('--analize_all', type=bool, default=False, help='Analyze all articles if True')
    ...
    if not api_key:
        logger.error("No se encontró la clave API en las variables de entorno (FIREWORKS_API_KEY)")
        sys.exit(1)
