...
def scrape_article_content(url, headers)->Dict:
    """
    ...
    try:
        ...
        title = soup.find('h2', class_='title').get_text(strip=True) if soup.find('h2', class_='title') else "No título"
        
        date = soup.find('time').get('datetime') if soup.find('time') else "No fecha"
        
        content_div = soup.find('div', class_='note_content')
        if content_div:
            for element in content_div(['script', 'style', 'iframe', 'ins', 'header', 'footer', 'nav']):
                element.decompose()
